...
def main():
    ...
    print('SELD score (early stopping metric): {:0.2f}'.format(student_seld_scr))
    ...
    print('SELD score (early stopping metric): {:0.2f}'.format(target_seld_scr))
    print('SED metrics: Error rate: {:0.2f}, F-score: {:0.1f}'.format(
    ...
    ## ------------ CLASS-WISE EVALUATION - STUDENT MODEL ------------------------------------
    if conf.metric['average'] == 'macro' and len(student_classwise_scr) > 0:
        print('\nClass-wise results for Student Model:')
        print('Class\tER\tF\tLE\tLR\tSELD_score')
        for cls_cnt in range(conf.input['num_classes']):
            print('{}\t{:0.2f}\t{:0.3f}\t{:0.2f}\t{:0.3f}\t{:0.3f}'.format(
                cls_cnt,
                student_classwise_scr[0][cls_cnt],
                student_classwise_scr[1][cls_cnt],
                student_classwise_scr[2][cls_cnt],
                student_classwise_scr[3][cls_cnt],
                student_classwise_scr[4][cls_cnt]))
    
    ## ------------ CLASS-WISE EVALUATION - TARGET MODEL ------------------------------------
    if conf.metric['average'] == 'macro' and len(target_classwise_scr) > 0:
        print('\nClass-wise results for Target Model:')
        print('Class\tER\tF\tLE\tLR\tSELD_score')
        for cls_cnt in range(conf.input['num_classes']):
            print('{}\t{:0.2f}\t{:0.3f}\t{:0.2f}\t{:0.3f}\t{:0.3f}'.format(
                cls_cnt,
                target_classwise_scr[0][cls_cnt],
                target_classwise_scr[1][cls_cnt],
                target_classwise_scr[2][cls_cnt],
                target_classwise_scr[3][cls_cnt],
                target_classwise_scr[4][cls_cnt]))
    
    ## ------------ CLASS-WISE COMPARISON ------------------------------------
    if conf.metric['average'] == 'macro' and len(student_classwise_scr) > 0 and len(target_classwise_scr) > 0:
        print('\nClass-wise Comparison (Target - Student):')
        print('Class\tER\tF\tLE\tLR\tSELD_score')
        for cls_cnt in range(conf.input['num_classes']):
            print('{}\t{:0.2f}\t{:0.3f}\t{:0.2f}\t{:0.3f}\t{:0.3f}'.format(
                cls_cnt,
                target_classwise_scr[0][cls_cnt] - student_classwise_scr[0][cls_cnt],
                target_classwise_scr[1][cls_cnt] - student_classwise_scr[1][cls_cnt],
                target_classwise_scr[2][cls_cnt] - student_classwise_scr[2][cls_cnt],
                target_classwise_scr[3][cls_cnt] - student_classwise_scr[3][cls_cnt],
                target_classwise_scr[4][cls_cnt] - student_classwise_scr[4][cls_cnt]))
